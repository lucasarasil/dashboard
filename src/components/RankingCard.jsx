...
const RankingCard = ({ clusters }) => {
  // Função para calcular score crítico (quanto maior, pior)
  const calculateCriticalScore = (cluster) => {
    return (
  ...
  const getD1Variation = (cluster, metric) => {
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-2 text-xs mt-2">
                  <div>
                    ...
                    <span className={`ml-1 font-medium ${cluster.atrasos > 20 ? 'text-red-600' : 'text-gray-700'}`}>
                      {cluster.atrasos}%
                    </span>
                  </div>
                  <div>
                    <span className="text-gray-500">Faltas:</span>
                    <span className={`ml-1 font-medium ${cluster.faltas > 7 ? 'text-red-600' : 'text-gray-700'}`}>
                      {cluster.faltas}%
                    </span>
                  </div>
                  <div>
                    <span className="text-gray-500">SLA:</span>
                    <span className={`ml-1 font-medium ${cluster.sla < 80 ? 'text-red-600' : 'text-gray-700'}`}>
                      {cluster.sla}%
                    </span>
                  </div>
                  <div>
                    <span className="text-gray-500">Produtividade:</span>
                    <span className={`ml-1 font-medium ${cluster.produtividadeApropriacao < 65 ? 'text-red-600' : 'text-gray-700'}`}>
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-1">
                      ...
                      <span className={`font-medium ${getTrendColor(atrasosVariation.direction)}`}>
                        {atrasosVariation.value > 0 ? '+' : ''}{atrasosVariation.value}%
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-1">
                      ...
                      <span className={`font-medium ${getTrendColor(slaVariation.direction)}`}>
                        {slaVariation.value > 0 ? '+' : ''}{slaVariation.value}%
                      </span>
                    </div>
                  </div>
                </div>
