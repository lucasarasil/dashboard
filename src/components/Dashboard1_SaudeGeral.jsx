...
const Dashboard1_SaudeGeral = () => {
  ...
  const criticalAlertsCount = filteredServices.filter(service => 
    service.status === 'critical' || service.hasAlert
  ).length;

  // Contadores por tipo
  const streetCallsCount = filteredServices.filter(s => s.serviceType === 'street_call').length;
  const appropriationsCount = filteredServices.filter(s => s.serviceType === 'appropriation').length;
  ...
  const filterServices = (search, filter, supervisorId = '', leaderId = '') => {
    ...
    if (supervisorId) {
      ...
      filtered = filtered.filter(service => supervisorBranches.includes(service.branch));
    }
