...
const ActionsTab = ({ service }) => {
  ...
  const manualActions = (service.logs || []).filter(log => log.type === 'manual_action');
  
  const actionTypes = [
    { value: 'assign', label: 'Atribuir Prestador', icon: UserIcon },
    { value: 'escalate', label: 'Escalar Serviço', icon: ExclamationTriangleIcon },
    { value: 'comment', label: 'Adicionar Comentário', icon: ClockIcon },
    { value: 'priority', label: 'Alterar Prioridade', icon: ExclamationTriangleIcon }
  ];
    ...
    <div className="h-full overflow-y-auto p-6">
      ...
      <div className="space-y-4">
        {manualActions.map((action) => (
          <div key={action.id} className="p-4 bg-green-50 border border-green-200 rounded-lg">
      ...
      {manualActions.length === 0 && !showActionForm && (
        <div className="text-center py-8">
          ...
          <p className="text-gray-500">Clique em "Nova Ação" para registrar uma ação manual.</p>
        </div>
      )}
    </div>
  );
