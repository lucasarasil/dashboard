...
const KPICards = ({ services }) => {
  const totalActive = services.length;
  const criticalCount = services.filter(s => s.status === 'critical' || s.hasAlert).length;
  const criticalPercentage = totalActive > 0 ? Math.round((criticalCount / totalActive) * 100) : 0;
  
  const slaMet = services.filter(s => s.slaProgress < 80).length;
  const slaPercentage = totalActive > 0 ? Math.round((slaMet / totalActive) * 100) : 0;
  
  // KPIs adicionais
  const appropriationsFinished = services.filter(s => s.serviceType === 'appropriation' && s.status === 'completed').length;
  const streetCallsOpen = services.filter(s => s.serviceType === 'street_call' && s.status === 'open').length;
  const alertsNotTreated = services.filter(s => s.hasAlert && !s.actionTaken).length;
  ...
  const kpis = [
    {
      ...
      borderColor: criticalPercentage > 20 ? 'border-red-200' : 'border-orange-200'
    },
    {
      title: '% SLA Cumprido',
      value: `${slaPercentage}%`,
    ...
    },
    {
      ...
      icon: UserGroupIcon,
      color: offlineProviders > 5 ? 'text-red-600' : offlineProviders > 3 ? 'text-yellow-600' : 'text-green-600',
      bgColor: offlineProviders > 5 ? 'bg-red-50' : offlineProviders > 3 ? 'bg-yellow-50' : 'bg-green-50',
      borderColor: offlineProviders > 5 ? 'border-red-200' : offlineProviders > 3 ? 'border-yellow-200' : 'border-green-200'
    },
    {
