...
const ServiceCard = ({ service, onClick, isListMode = false }) => {
  ...
  const getServiceIcon = (serviceName) => {
    ...
    } else if (serviceName.includes('Fusível') || serviceName.includes('Bateria') || serviceName.includes('Motor')) {
  ...
  const getServiceAddressType = (serviceName, serviceType) => {
    if (serviceType === 'appropriation') {
      return { type: 'Cadastro', icon: BuildingOfficeIcon, color: 'text-purple-600', bgColor: 'bg-purple-100' };
    }
    
    // Tipos de serviço baseados no nome
    if (serviceName.includes('Fusível') || serviceName.includes('Bateria') || serviceName.includes('Motor')) {
      return { type: 'Casa', icon: HomeIcon, color: 'text-blue-600', bgColor: 'bg-blue-100' };
    } else if (serviceName.includes('Pneu') || serviceName.includes('Freio') || serviceName.includes('Farol')) {
      return { type: 'Rua', icon: MapIcon, color: 'text-green-600', bgColor: 'bg-green-100' };
    } else if (serviceName.includes('Carburador') || serviceName.includes('Acelerador')) {
      return { type: 'Satélite', icon: SunIcon, color: 'text-yellow-600', bgColor: 'bg-yellow-100' };
    } else {
      return { type: 'Pernoite', icon: MapPinIcon, color: 'text-indigo-600', bgColor: 'bg-indigo-100' };
    }
  };
  ...
  if (isListMode) {
    const addressType = getServiceAddressType(service.serviceName, service.serviceType);
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="flex-1">
              <div className="flex items-center space-x-3 mb-1">
                ...
                <span className={`status-badge ${getStatusColor(service.status)} flex items-center space-x-1 text-xs font-semibold`}>
              ...
              <p className="text-sm font-medium text-gray-700 mb-2">{service.serviceName}</p>
              
              {/* Tipo de endereço */}
              <div className="flex items-center space-x-2 mb-2">
                <div className={`flex items-center space-x-1 px-2 py-1 rounded-full ${addressType.bgColor}`}>
                  <addressType.icon className={`h-3 w-3 ${addressType.color}`} />
                  <span className={`text-xs font-medium ${addressType.color}`}>
                    {addressType.type}
                  </span>
                </div>
                <span className="text-xs text-gray-500">{service.vehiclePlate}</span>
    ...
    >
      <div className="flex items-start justify-between mb-3">
        ...
        <div className="flex flex-col items-end space-y-1">
          <span className={`status-badge ${getStatusColor(service.status)} flex items-center space-x-1`}>
            {getStatusIcon(service.status)}
            <span className="capitalize">{service.status.replace('_', ' ')}</span>
          </span>
        </div>
