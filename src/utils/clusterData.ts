...
export const calculateClusterStatus = (cluster: any) => {
 ...
 if (
  cluster.apropriacoesFinalizadas <= criticalThresholds.apropriacoesFinalizadas
 )
  criticalCount++;
 else if (
  cluster.apropriacoesFinalizadas <= attentionThresholds.apropriacoesFinalizadas
 )
  attentionCount++;
...
export const calculateHealthScore = (clusters: any[]) => {
 if (!clusters || clusters.length === 0) return 0;

 // Pesos para cada métrica (ajustar depois conforme necessidade)
 const weights = {
  sla: 0.3, // 30% - SLA é crítico
  atrasos: 0.2, // 20% - Atrasos impactam muito
  faltas: 0.15, // 15% - Faltas prejudicam operação
  adesao: 0.15, // 15% - Adesão à jornada importante
  produtividade: 0.1, // 10% - Produtividade em apropriações
  dimensionamento: 0.1, // 10% - Aderência ao dimensionamento
 };
 ...
 clusters.forEach((cluster: any) => {
  ...
  const dimensionamentoPercentual =
   (cluster.motoristas / cluster.motoristasNecessarios) * 100;
...
export const supervisores = [
 ...
 },
 {
  id: "sup-evandro",
  nome: "Evandro",
  lideres: [{ id: "lid-bruno", nome: "Bruno DE.", clusters: ["cluster-2"] }],
 },
 {
  id: "sup-ana-lima",
  nome: "Ana Lima",
 ...
 },
 {
  id: "sup-joao-costa",
  nome: "João Costa",
  lideres: [{ id: "lid-maria", nome: "Maria Santos", clusters: ["cluster-4"] }],
 },
 {
  id: "sup-lucia",
  nome: "Lucia Ferreira",
